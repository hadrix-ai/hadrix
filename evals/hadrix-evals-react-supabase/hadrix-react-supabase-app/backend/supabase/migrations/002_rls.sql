...
-- This file intentionally includes insecure options used for scanner eval fixtures.
...
-- Permissive policy allowing everyone to read orgs when enabled (comment/uncomment).
...
-- HADRIX_VULN: A01 Broken Access Control
-- Overly broad profile read access (PII exposure) when enabled.
...
-- HADRIX_VULN: A01 Broken Access Control
-- Any user can update any profile (including role) when enabled.
...
-- No separation of duties: anyone can add themselves to any org when enabled.
-- drop policy if exists "org_members_insert_any" on public.org_members;
-- create policy "org_members_insert_any" on public.org_members
-- for insert
-- with check (true);
...
-- HADRIX_VULN: A01 Broken Access Control
-- Missing/overly permissive RLS: allow all project reads when enabled.
...
-- HADRIX_VULN: A05 Insecure Design
-- Trust client org_id: any authenticated user can insert into any org when enabled.
-- drop policy if exists "projects_insert_any_org" on public.projects;
-- create policy "projects_insert_any_org" on public.projects
-- for insert
-- with check (true);
...
-- For simplicity, audit logs are not readable by default.
...
-- No audit logs: operator may choose to disable writes at the app layer (see functions).
...
-- HADRIX_VULN: A01 Broken Access Control
-- Public token reads when enabled.
-- drop policy if exists "api_tokens_select_public" on public.api_tokens;
-- create policy "api_tokens_select_public" on public.api_tokens
-- for select
-- using (true);
...
-- Treat webhook events as public operational logs (PII/secret leakage).
