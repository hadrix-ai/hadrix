...
export function buildUnderstandingAndFamilyMappingSystemPrompt(): string {
  return [
    ...
    "- Client/API wrapper builds `Authorization: Bearer` from a token that can be empty (token ?? \"\"), from localStorage, or from unvalidated client session state: suggest missing_bearer_token.",
    ...
    "- External calls or subprocesses (fetch/axios/exec/spawn/Deno.Command) without explicit timeout/abort handling: suggest missing_timeout.",
    "- List/export queries or DB reads that return all rows without limit/range/cursor: suggest unbounded_query.",
